...
QByteArray __DvtSendMail_EncodeBody(QString *charset,QString *encoding,
				   const QString &str)
{
  QByteArray raw;
  int index=0;
...
QByteArray __DvtSendMail_EncodeHeader(const QString &str)
{
  if(__DvtSendMail_IsAscii(str)) {
    return str.toUtf8();
  }
...
QByteArray __DvtSendMail_EncodeAddress(const QString &str,bool *ok)
{
  //
  ...
  *ok=true;
  ...
  return __DvtSendMail_EncodeHeader(name)+" <"+addr.toUtf8()+">";
}
  ...
  *err_msg="";
  ...
  if(from_addr.isEmpty()) {
    *err_msg+=QObject::tr("You must supply a \"from\" address")+"\n";
    ...
    from_addr_enc=__DvtSendMail_EncodeAddress(from_addr,&ok);
    if(!ok) {
  ...
  for(int i=0;i<to_addrs.size();i++) {
    to_addrs_enc.push_back(__DvtSendMail_EncodeAddress(to_addrs.at(i),&ok));
    if(!ok) {
  ...
  for(int i=0;i<cc_addrs.size();i++) {
    cc_addrs_enc.push_back(__DvtSendMail_EncodeAddress(cc_addrs.at(i),&ok));
    if(!ok) {
  ...
  for(int i=0;i<bcc_addrs.size();i++) {
    bcc_addrs_enc.push_back(__DvtSendMail_EncodeAddress(bcc_addrs.at(i),&ok));
    if(!ok) {
  ...
  QString charset;
  QString encoding;
  QByteArray raw=__DvtSendMail_EncodeBody(&charset,&encoding,body);
  ...
  msg+="Subject: "+__DvtSendMail_EncodeHeader(subject)+"\r\n";
  ...
  //
  // FIXME: proc->exitStatus() falsely returns QProcess::CrashExit on
  //        LinuxMint 21. Why??
  /*
